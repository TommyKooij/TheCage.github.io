...
const ShowHabuchi = () => {
  const [habuchiList, setHabuchiList] = useState([]);
  const habuchiCollection = collection(db, "Habuchi: Crimson Sheen");

  useEffect(() => {
    const getHabuchi = async () => {
      const data = await getDocs(habuchiCollection);

      data.forEach((doc) => {
        setHabuchiList(data.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
  ...
  return (
    <Site>
      <Header />
      <TextContainer>
        Habuchi: Crimson Sheen
        {/* {habuchiList &&
          habuchiList.map((book) => {
